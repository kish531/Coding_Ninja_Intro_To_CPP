...
Output Format:The output contains the string after compression printed in single line.
Note:You are not required to print anything. It has already been taken care of. Just implement the given function.
...
string getCompressedString(string &input) {
    ...
    while(startIndex<input.length()){
        while(endIndex<input.length() && input[startIndex]==input[endIndex]){
            endIndex++;
        }
        int totalCount=endIndex-startIndex;
        if(totalCount!=1){
            compressedString+=input[startIndex];
            compressedString+=(char)(totalCount+'0');
        }else{
            compressedString+=input[startIndex];
        }
        startIndex=endIndex;
